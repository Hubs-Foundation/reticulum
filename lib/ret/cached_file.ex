defmodule Ret.CachedFile do
  ...
  def fetch(cache_key, loader) do
    Repo.transaction(fn -> 
      ...
      << lock_key::little-signed-integer-size(64), _::binary >> = :crypto.hash(:sha256, key)

      Ecto.Adapters.SQL.query!(Repo, "select pg_advisory_xact_lock($1);", [lock_key])

      case ResolvedMediaAsset
        |> where(cache_key: ^cache_key)
        |> Repo.one() do

        %ResolvedMediaAsset{file_uuid: file_uuid, file_content_type: file_content_type, file_key: file_key} ->
        ...
        nil ->
          ...
          %CachedFile
          |> changeset(%{ cache_key: cache_key, file_uuid: file_uuid, file_key: file_key, file_content_type: content_type})
          |> Repo.insert!
  ...
  def vacuum do
    ...
    from(r in ResolvedMediaAsset, where: r.created_at() < ^one_day_ago)
    |> Repo.delete_all
  end
end

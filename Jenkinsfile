    ...
    stage('build') {
      steps {
        ...
        script {
            ...
            def gitMessageClient = sh(returnStdout: true, script: "curl --silent 'https://api.github.com/repos/mozilla/mr-social-client/commits?per_page=1' | jq -r '.[0] | .commit.message'").trim()
